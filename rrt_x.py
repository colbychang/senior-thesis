...
class RRTX:
    ...
    def planning(self, animation=True):
        ...
        if create_gif:
            writer = imageio.get_writer('rrtx.mp4', fps=3)
    ...
    def update_obstacles(self, radius=1.5):
        ...
        # check if robot can sense a fake obstacle and if so, remove it
        obs_to_keep = []
        ...
        # check if robot can sense a hidden obstacle and if so, add it
        obs_to_keep = []
    ...
    def remove_obstacle(self, removed_obs):
        ...
        # check edges to see if they're affected by this removal
        for edge in self.edge_list:
